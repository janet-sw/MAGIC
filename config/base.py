...
def get_config():
    ...
    # batch_size * num_gpus`.
    sample.num_batches_per_epoch = 2 #16 #32 #64 #128 #256
    # save interval
    ...
    train.sample_path = "/data/dpo_output/2025-03-29-20-18-01"
    # path to the directory containing the json file for feedbacks
    train.json_path = "/media/janet/SKIN-D3PO/feedback/20class_0.5_ratio_3100_w_bodyparts_pairs=1024_medgemma/"
    ...
    ############ MAGIC Config ############
    # learned emmbedding 
    config.embed_path = '/media/janet/DermDPO/models/textual_inversion_weights/fitzpatrick17k/aggregated_embed_sd2_1_base.pt'
    # load lora weights
    config.lora_weights = '/media/janet/DermDPO/models/lora_weights/fitzpatrick17k/updated_pytorch_lora_weights.safetensors' ### run scripts/lora_weights_converter.py to convert LoRA weights
    # path to input disease images
    config.image_dir = '/data/derm_data/Fitzpatrick17k/finalfitz17k'
    # metadata
    config.metadata_path = '' ### add body part info if available so that generation is conditioned on body parts
    # rank
    config.rank = 32 # make sure it's same as the rank in preliminary training
