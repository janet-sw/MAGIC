...
import numpy as np
import magic_pytorch.prompts
from magic_pytorch.diffusers_patch.pipeline_with_logprob import pipeline_with_logprob
import torch
...
def main(_):
    ...
    if config.use_lora:
        ...
        for name in pipeline.unet.attn_processors.keys():
            ...
            lora_attn_procs[name] = LoRAAttnProcessor(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, rank=config.rank)
        for name in pipeline.unet.attn_processors.keys():
            cross_attention_dim = (
                None if name.endswith("attn1.processor") else pipeline.unet.config.cross_attention_dim
            )
    ...
    prompt_fn = getattr(magic_pytorch.prompts, config.prompt_fn)
    ...
    condition_mapping = {
                'acne': 'xxxacnxxx',
                'actinic keratosis': 'xxxactxxx',
                'allergic contact dermatitis': 'xxxallxxx',
                'basal cell carcinoma': 'xxxbasxxx',
                'eczema': 'xxxeczxxx',
                'erythema multiforme': 'xxxeryxxx',
                'folliculitis': 'xxxfolxxx',
                'granuloma annulare': 'xxxgraxxx',
                'keloid': 'xxxkelxxx',
                'lichen planus': 'xxxlicxxx',
                'lupus erythematosus': 'xxxlupxxx',
                'melanoma': 'xxxmelxxx',
                'mycosis fungoides': 'xxxmycxxx',
                'pityriasis rosea': 'xxxpitxxx',
                'prurigo nodularis': 'xxxpruxxx',
                'psoriasis': 'xxxpsoxxx',
                'sarcoidosis': 'xxxsarxxx',
                'scabies': 'xxxscaxxx',
                'squamous cell carcinoma': 'xxxsquxxx',
                'vitiligo': 'xxxvitxxx',
                }
    
    # with open(config.metadata_path, 'r') as f:
    #     body_part_dist = json.load(f)
    
    def create_prompt(source_condition, body_part=None, template=None, gross_body_part=None):
        if gross_body_part != None:
            gross_body_part = gross_body_part.split(', ')
        
        try:
            target_condition = random.choice([c for c in condition_mapping.keys() 
                                if c != source_condition
                                and any(part in body_part_dist[c] for part in gross_body_part)])
            template = '<condition> around the <body_part> of a human'
            output = template.replace("<body_part>", body_part).replace("<condition>", condition_mapping[target_condition])

        except:
            target_condition = random.choice([c for c in condition_mapping.keys() 
                                if c != source_condition])
            template = '<condition> on the skin of a human'
            output = template.replace("<condition>", condition_mapping[target_condition])
    ...
    for i in tqdm(
        ...
        # retrieve prepared prompt-caption pairs
        image_prompt_pairs, metadata = zip(*[prompt_fn() for _ in range(config.sample.batch_size)])
                          
        init_images_paths = [entry["image_path"] for entry in image_prompt_pairs]
        full_image_paths = [os.path.join(image_dir, path) for path in init_images_paths]
        init_images = [load_my_image(image_path) for image_path in full_image_paths]
        prompts = [create_prompt(entry["label"]) for entry in image_prompt_pairs]
        # prompts = [create_prompt(entry["label"], entry["body_part"], entry["gross_body_part"]) for entry in image_prompt_pairs]
    
        # we set the prompts to be the same
        # prompts1 = ["1 hand"] * config.sample.batch_size 
        prompts7 = prompts6 = prompts5 = prompts4 = prompts3 = prompts2 = prompts1 = prompts
        init_image7 = init_image6 = init_image5 = init_image4 = init_image3 = init_image2 = init_image1 = init_images
        image_paths7 = image_paths6 = image_paths5 = image_paths4 = image_paths3 = image_paths2 = image_paths1 = init_images_paths
        
        total_prompts.extend(prompts1)
        total_images.extend(image_paths1)
        ...
        if (i+1)%config.sample.save_interval ==0 or i==(config.sample.num_batches_per_epoch-1):
            ...
            with open(os.path.join(save_dir, f'image{accelerator.process_index}.json'),'w') as f:
                json.dump(total_images, f)
