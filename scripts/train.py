...
from magic_pytorch.diffusers_patch.ddim_with_logprob import ddim_step_with_logprob
import torch
...
def main(_):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="magic-pytorch", config=config.to_dict(), init_kwargs={"wandb": {"name": config.run_name}}
        )
    logger.info(f"\n{config}")
    ...
    pipeline = StableDiffusionPipeline.from_pretrained(config.pretrained.model, 
    ...
    combinations_list = list(combinations(range(2), 2))
    num_timesteps = samples["timesteps"].shape[1]
    total_batch_size = human_prefer.shape[0]
    strength = config.strength ### we are doing img2img
    ...
    for epoch in range(config.num_epochs):
        ...
        perms = torch.stack(
            [torch.randperm(int(num_timesteps * strength)) for _ in range(total_batch_size)]
        )
        ...
        for i in tqdm(range(0,total_batch_size,config.train.batch_size),
            ...
            for each_combination in combinations_list:
                ...
                for j in tqdm(
                    range(int(num_timesteps * strength)),
                    desc="Timestep",
                    position=3,
                    leave=False,
                    disable=not accelerator.is_local_main_process,
