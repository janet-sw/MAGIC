...
def remove_mid_file(save_dir, num_processes):
    filename = [f'prompt{i}.json' for i in range(num_processes)] + [f'sample{i}.pkl' for i in range(num_processes)] + [f'{i}.txt' for i in range(num_processes)] + [f'image{i}.json' for i in range(num_processes)]
...
def post_processing(save_dir, num_processes):
    ...
    images = []
    for i in range(num_processes):
        with open(os.path.join(save_dir, f'image{i}.json'), 'r') as f:
            images_ = json.load(f)
            images += images_
    print('---------write image path---------')
    with open(os.path.join(save_dir, 'image.json'), 'w') as f:
        json.dump(images, f)
